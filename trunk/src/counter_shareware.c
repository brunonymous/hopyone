 ...
 */
static Sint32
RegOpenKeyEx (HKEY h_key, const char* lpSubKey, DWORD ulOptions, Sint32 samDesired,
              Uint32* phkResult)
{
 ...
 */
static Sint32
RegCreateKeyEx (HKEY h_key, const char* lpSubKey, DWORD Reserved, char* lpClass,
                DWORD dwOptions, Sint32 samDesired,
                LPSECURITY_ATTRIBUTES lpSecurityAttributes, Uint32 *phkResult,
 ...
 */
static Sint32
RegSetValueEx (HKEY h_key, const char* lpValueName, DWORD Reserved, DWORD dwType,
...
counter_load_file (void)
{
  ...
  pos += 4;
  num_of_games = nb;
  GameCount = (TLKGameInfo *) memory_allocation (sizeof(TLKGameInfo) * (num_of_games + 1));
  if (GameCount == NULL)
    {
      return FALSE;
    } 
