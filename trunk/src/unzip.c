...
extern FILE *ZEXPORT
unzReadCurrentFile_GetFP (unzFile file, voidp buf, unsigned len)
{
  //int err=UNZ_OK;
  //uInt iRead = 0;
  unz_s *s;
  ...
  if ((pfile_in_zip_read_info->read_buffer == NULL))
    return (FILE *) UNZ_END_OF_LIST_OF_FILE;
  if (len == 0)
    return 0;
      ...
      if ((pfile_in_zip_read_info->stream.avail_in == 0) &&
          (pfile_in_zip_read_info->rest_read_compressed > 0))
        {
          ...
          if (fseek (pfile_in_zip_read_info->file,
                     ...
                     pfile_in_zip_read_info->byte_before_the_zipfile,
                     SEEK_SET) != 0)
            return (FILE *) UNZ_ERRNO;
          return pfile_in_zip_read_info->file;
        }
